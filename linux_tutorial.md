...
* use right click + E to open terminal in the current folder (open in GUI file manager)


* using precompiled (portable) executable application:
	- extract the application folder to some location, there should an executable file there
	- in ~/bin (or other folder in the PATH) create a soft link to the executable file, make the link
	...
	[Desktop Entry]
	Name=Modern csv
	Comment=Open csv files
	Exec=/home/vlad/bin/mcsv %U
	Type=Application
	```
